...
impl GrpcService {
    ...
    async fn geyser_loop(
        ...
        loop {
            tokio::select! {
                Some(message) = messages_rx.recv() => {
                    ...
                    // 仅保留两类：Processed Slot & Account
                    let pass = match &message {
                        Message::Slot(s) if s.status == SlotStatus::Processed => true,
                        Message::Account(_) => true,
                        _ => false,
                    };
                    if !pass { continue; }

                    // 记录 slot 指标（仅 processed）
                    if let Message::Slot(slot_message) = &message {
                        metrics::update_slot_plugin_status(slot_message.status, slot_message.slot);
                    }

                    processed_messages.push((msgid_gen.next(), message));
        ...
        //                     if processed_messages.len() >= PROCESSED_MESSAGES_MAX || !confirmed_messages.is_empty() || !finalized_messages.is_empty() {
        //                         let _ = broadcast_tx.send((CommitmentLevel::Processed, processed_messages.into()));
        //                         processed_messages = Vec::with_capacity(PROCESSED_MESSAGES_MAX);
        //                         processed_sleep
        //                             .as_mut()
